...
describe("A read cursor", () => {
  test("is undefined if not set", async () => {
    const actual = await page.evaluate(async () => {
      const alice = await aliceChatManager.connect()
      return alice.readCursor
    })
  ...
  test("sets unread count", async () => {
    // before a cursor is set, the unread count is undefined
    ...
    expect(initial.unread).toBeUndefined
    expect(initial.messageAt).toBeUndefined
