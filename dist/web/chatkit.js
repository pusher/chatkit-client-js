...
var appendQueryParam = function appendQueryParam(key, value, url) {
  var _split = split('?', url),
      _split2 = slicedToArray(_split, 2),
      before = _split2[0],
      after = _split2[1];

  return before + '?' + (after ? after + '&' : '') + urlEncode(defineProperty({}, key, value));
};
...
var PresenceSubscription = function () {
  function PresenceSubscription(options) {
    ...
    this.onCameOnline = function (user) {
      if (_this.hooks.userCameOnline) {
        _this.hooks.userCameOnline(user);
      }
      compose(forEach(function (sub) {
        ...
        return sub.hooks.userCameOnlineInRoom !== undefined;
      }), values)(_this.roomSubscriptions);
    };

    this.onWentOffline = function (user) {
      if (_this.hooks.userWentOffline) {
        _this.hooks.userWentOffline(user);
      }
      compose(forEach(function (sub) {
        return _this.roomStore.get(sub.roomId).then(function (room) {
          if (contains$1(user.id, room.userIds)) {
        ...
        return sub.hooks.userWentOfflineInRoom !== undefined;
      }), values)(_this.roomSubscriptions);
    };
...
var CurrentUser = function () {
  function CurrentUser(_ref) {
    ...
    this.establishUserSubscription = function (hooks) {
      ...
      return _this.userSubscription.connect().then(function (_ref5) {
        var user = _ref5.user,
            basicRooms = _ref5.basicRooms;
